...
type ProStatus = { isPro: boolean } | null;
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      {/* Sidebar */}
      <nav className="w-60 shrink-0 flex flex-col gap-1 py-6 px-3 overflow-y-auto"
        ...
        {FREE_ITEMS.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon className="w-4 h-4 shrink-0" style={{ color: "var(--success)" }} />
              {label}
            </button>
          );
        })}
        ...
        {/* Pro nav items */}
        <p className="text-xs font-semibold uppercase tracking-widest px-3 mb-2 flex items-center gap-2"
          style={{ color: "rgba(255,255,255,0.35)" }}>
          Pro
          {proStatus && !proStatus.isPro && (
        ...
        {PRO_ITEMS.map(({ href, label, icon: Icon }) => {
          const active = pathname === href;
          const locked = proStatus !== null && !proStatus.isPro;
          return (
            <button
              key={href}
            ...
            >
              <Icon className="w-4 h-4 shrink-0"
                style={{ color: active ? "var(--accent)" : locked ? "rgba(255,255,255,0.25)" : "var(--accent)" }} />
              {label}
            </button>
          );
        })}
